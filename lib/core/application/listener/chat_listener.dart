...
class ChatListener extends ChangeNotifier {
  // list of messages
  ...
  Future<List<MessageModel>> loadMessagesFromDB({required String chatId}) async {
    // open the box of this chatID
    await Hive.openBox('${AppData.chatMessagesBox}$chatId');

    final messageBox = Hive.box('${AppData.chatMessagesBox}$chatId');

    final newData = messageBox.keys.map((e) {
      final message = messageBox.get(e);
      final messageData = MessageModel.fromMap(Map<String, dynamic>.from(message));
  ...
  Future<void> deletChatMessages({required String chatId}) async {
    ...
    if (!Hive.isBoxOpen('${AppData.chatMessagesBox}$chatId')) {
      // open the box
      await Hive.openBox('${AppData.chatMessagesBox}$chatId');

      // delete all messages in the box
      await Hive.box('${AppData.chatMessagesBox}$chatId').clear();

      // close the box
      await Hive.box('${AppData.chatMessagesBox}$chatId').close();
    } else {
      // delete all messages in the box
      await Hive.box('${AppData.chatMessagesBox}$chatId').clear();
  ...
  Future<void> sendMessageAndWaitForResponse({
    ...
    chatSession.sendMessageStream(content).asyncMap((event) {
      ...
      if(assistantMessage.message.toString().isEmpty){
        assistantMessage.message=StringBuffer('Something went wrong (Try again in sometime)');
      }
  ...
  Future<void> saveMessagesToDB({
    ...
    final chatHistoryBox = LocalStorage().getChatHistory();
  ...
  static initHive() async {
    ...
    Hive.init(dir.path);
    await Hive.initFlutter(AppData.geminiDB);
    ...
    if (!Hive.isAdapterRegistered(0)) {
      ...
      await Hive.openBox<ChatHistory>(AppData.chatHistoryBox);
    }
  }
}
